...
'''
DEPRICIATED, MAY BEHAVE IN UNSTABLE WAYS DUE TO NEWER CHANGES
...
def double_exponential_fit(x, y, p0=-1, xl=-1, xr=-1, xstart=-1, xstop=-1):
    ...
    try:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            pl, plconv = fit(symm_exp, x[xstart:xl], y[xstart:xl], p0=p0)
    ...
    try:
        pr, prconv = fit(symm_exp, x[xr:xstop], y[xr:xstop], p0=p0)
